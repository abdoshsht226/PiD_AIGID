...
EARLY_STOP_PATIENCE = 10  # EDIT: stop if val accuracy doesn't improve for this many consecutive shard-validations
...
def train():
        ...
        for batch_idx, (pil_images, labels) in enumerate(train_gen, start=1):  # this is the main training loop and it eneminates the data from the generator (that yeilds batches) and passes it to the model
            ...
            try:
                ...
                for img in pil_images:
                    residual = pid_module.apply_pid_algorithm(img, train=True)  # EDIT: random crop (augmentation) for training
            ...
            if batch_in_shard_counter >= 100:
                ...
                run_val_cycle(model, current_active_shard, epoch, optimizer, loss.item())  # run validation after each shard and save the curr model as the new checkpoint (rolling, for resuming)

                # EDIT: overfitting protection -- stop training if validation
                # accuracy hasn't improved in EARLY_STOP_PATIENCE consecutive
                # shards. Crash-safe: no_improve_count is persisted to
                # training_state.json, so resuming after a Colab disconnect
                # doesn't reset the patience counter.
                if model_manager.should_early_stop(patience=EARLY_STOP_PATIENCE):
                    print(f"\n[EARLY STOP] No validation improvement for {EARLY_STOP_PATIENCE} shards. Stopping training.")
                    print("[INFO] Use model_manager.load_best_model(DEVICE) to load the best checkpoint for testing/inference.")
                    return
    ...
    print("\n[INFO] Training finished. Use model_manager.load_best_model(DEVICE) to load the best-validation checkpoint for testing/inference.")
...
def run_val_cycle(model, shard_idx, epoch, optimizer, latest_loss):
   ...
   evaluate the model on a cached validation set
    ...
    model_manager.save_checkpoint(model, optimizer, shard_idx, epoch, latest_loss)  # save the current state at the checkpoint (rolling, for resuming)
    model_manager.save_best_checkpoint(model, optimizer, shard_idx, epoch, accuracy)  # EDIT: separately save only if this beats the best val accuracy so far
