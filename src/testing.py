...
def test():
    ...
    """
    # EDIT: evaluate the best-validation-accuracy checkpoint, not just
    # whatever the last saved shard happened to be (which could be overfit).
    # Falls back to the rolling "latest" checkpoint if no best_model.pth
    # exists yet (e.g. you're testing before any validation cycle ran).
    try:
    ...
    for batch_idx, (images, labels) in enumerate(test_gen, start=1):
        try:
            processed_list = []
            for img in images:
                residual = pid_module.apply_pid_algorithm(img, train=False)  # EDIT: deterministic center crop for testing
